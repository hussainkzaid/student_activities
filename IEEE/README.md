...
This repository collects a series of weekly, hands-on exercises building core data analysis skills in Python — from NumPy and Pandas fundamentals through data cleaning, visualization, exploratory data analysis (EDA), and business intelligence dashboarding with Power BI.
...
| **week7-Power_BI** | Business Intelligence Dashboard | A single-page Power BI dashboard (`Telco_Customer_Churn_Dashboard.pbix`) analyzing telecom customer churn — KPI cards (total churned, churn rate, revenue, lost revenue), charts by contract type, payment method, service add-ons, and churn reasons, plus a tenure-vs-charge scatter plot. See the folder's own `README.md` for details. |
...
Week 7 additionally requires **Power BI Desktop** (Windows, free from Microsoft) to open, edit, or refresh the `.pbix` dashboard file.

## Notes

- This repo is built for **educational purposes** as part of an IEEE learning track, progressing from basic Python data tools toward full exploratory data analysis workflows and business intelligence dashboarding.
